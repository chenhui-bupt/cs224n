...
class ParserModel(Model):
    ...
    def add_placeholders(self):
        ...
        self.labels_paceholder = tf.placeholder(tf.float32, [None, self.config.n_classes])
    ...
    def create_feed_dict(self, inputs_batch, labels_batch=None, dropout=0):
        ...
        if labels_batch is not None:
            feed_dict[self.labels_paceholder] = labels_batch
        ### END YOUR CODE
        return feed_dict
    ...
    def add_embedding(self):
        ...
        inputs = tf.nn.embedding_lookup(pretrained_embeddings, self.input_placeholder)
        embeddings = tf.reshape(inputs, [-1, self.config.n_features * self.config.embed_size])
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        ### YOUR CODE HERE
        xavier_mat = xavier_weight_init()
        W1 = xavier_mat([self.config.n_features * self.config.embed_size, self.config.hidden_size])
        b1 = tf.random_uniform([self.config.hidden_size])
        h = tf.nn.relu(tf.matmul(x, W1) + b1)
        h_drop = tf.nn.dropout(h, 1 - self.dropout_placeholder)
        W2 = xavier_mat([self.config.hidden_size, self.config.n_classes])
    ...
    def add_loss_op(self, pred):
        ...
        """
        ### YOUR CODE HERE
        loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=self.labels_paceholder, logits=pred))
        ### END YOUR CODE
        return loss
